<template>
  <div>
    ...
    >
      <el-form label-position="left" label-width="100px">
        <el-form-item label="文件夹名称">
          <el-input v-model="folderName" max="100" placeholder="请输入文件夹名称" />
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="提示" :visible.sync="showProjectMove" width="30%" center>
      <span
        style="font-size: large"
      >确定将项目 {{ dropData.projectName }} 移动到文件夹 {{ dropData.folderName }} 下吗？</span>
      ...
      <el-table border style="width: 100%;" :data="openFolderInfo" fit>
        ...
        >
          <template slot-scope="scope">
            ...
            <el-popover style="margin-left:10px" placement="left" width="500px">
              <div>
      ...
      if (!this.folderNewName.trim()) {
        ...
        if (this.folderInfo.folder_name !== this.folderNewName) {
          if (fs.existsSync(`${config.projectsPath}/${this.folderNewName}`)) {
            ...
            this.$parent.delDir(
              `${config.projectsPath}/${this.folderInfo.folder_name}`
            );
            this.$store.commit(
